...
function getBotUsername(ctx: any): string {
  const raw =
    ctx.botInfo?.username ||
    ctx.me ||
    ctx.telegram?.botInfo?.username ||
    ctx.bot?.botInfo?.username ||
    "";
...
function withMentionGate(handler: Handler): Handler {
...
export function registerHandlers({
  ...
  command("continue", async (ctx: any) => {
    const locale = localeOf(ctx.chat.id);
  ...
  command("language", async (ctx: any) => {
  ...
  command("interrupt", async (ctx: any) => {
  ...
  command("cron_now", async (ctx: any) => {
    const locale = localeOf(ctx.chat.id);
    try {
