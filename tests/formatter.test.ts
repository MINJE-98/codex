...
test("formatPtyOutput preserves fenced code blocks for Telegram MarkdownV2", () => {
  const rendered = formatPtyOutput(
    [
      "Run this:",
      "",
...
test("formatPtyOutput preserves inline code for Telegram MarkdownV2", () => {
  const rendered = formatPtyOutput('Use `python3 -c "print(123)"` first.');

  assert.equal(rendered, 'Use `python3 -c "print(123)"` first\\.');
});
