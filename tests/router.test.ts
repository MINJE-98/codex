...
test("router routes explicit GitHub commands to github skill", async () => {
  ...
  const route = await router.routeMessage('/gh commit "feat: migrate"', {
    chatId: 1
  });
  ...
  assert.deepEqual(route, {
    target: "skill",
    skill: "github",
    payload: '/gh commit "feat: migrate"'
  });
});

test("router does not auto-route plain GitHub mentions to github skill", async () => {
  const router = new Router({
    skills: {
      github: createSkill(() => true),
      mcp: createSkill(() => false)
  ...
  const route = await router.routeMessage(
    "github.com/fivetaku/insane-search 설치해줘",
    {
      chatId: 1
    }
  );
  ...
  assert.deepEqual(route, {
    target: "pty",
    prompt: "github.com/fivetaku/insane-search 설치해줘"
  });
});
