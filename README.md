...
This fork is published at `https://github.com/MINJE-98/codex` and is adapted for Codex SDK/CLI, MCP commands, and explicit GitHub skill routing.
...
```bash
git clone https://github.com/MINJE-98/codex.git
cd codex
npm install
cp .env.example .env
```
...
Use real tokens only in your local `.env` file or deployment secret store. Do not paste live bot tokens, GitHub tokens, session files, or other credentials into committed files.
...
- Explicit commands always go straight to the matching subagent:
  - `/gh ...` -> GitHub skill
  - `/mcp ...` -> MCP skill
- Plain text, including GitHub URLs and general repo instructions, stays in the Codex conversation unless it starts with an explicit command such as `/gh` or `/mcp`
...
- plain-text write requests are handled as normal Codex prompts; use `/gh ...` for GitHub writes
